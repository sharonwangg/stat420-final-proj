...
Below, we will be fitting the additive and interactive models for the data.
...
(back_pvalue = anova(back_aic, int_model)[2, "Pr(>F)"])
(forward_aic_pvalue = anova(forward_aic, int_model)[2, "Pr(>F)"])
(forward_bic_pvalue = anova(forward_bic, int_model)[2, "Pr(>F)"])
```
- The model created from backwards AIC/BIC predicts better than the interaction model with a p-value of `r back_pvalue`. 
...
Based on the boxcox plot, it seems like we should be using a logarithmic transformation so we start by taking the log of the response variable.
...
Now that we have all the AIC and BIC determined models with a log transformation of the response variable, we can take a look at the boxcox plots. 
...
These boxcox plots indicate that we made the right transformations so now we can determine which of these models are the best.
...
Based on these p-values from the anova tests, we should compare the backwards AIC model and the forward AIC model.

```{r}
(anova(forward_aic_log, back_aic_log_model)[2, "Pr(>F)"])
```

Based on the p-value of this anova test, we can see that the we should be choosing the smaller model which in this case is the foward_aic_log model. Let us set the forward_aic_log model as the final model we will be using to check the assumptions we are making of this model.

```{r}
final_model = forward_aic_log
```
...
# Breusch-Pagan Test
library(lmtest)
as.vector(bptest(final_model)$p.value)
```
...
# QQ Plot
qqnorm(final_model$residuals, main = "Normal Q-Q Plot of Best Model", pch = 20)
qqline(final_model$residuals, lwd = 2, col = "red")

# Shapiro-Wilkes Test
shapiro.test(final_model$residuals)$p.value
```
...
cleaned_log_model = step(
  clean_int_log_model,
  scope = total_confirmed ~ (school_closing + cancel_public_events + stay_at_home_requirements + mobility_parks + mobility_transit_stations)^2,
  direction = "forward", trace = 0)

shapiro.test(cleaned_log_model$residuals)$p.value
as.vector(bptest(cleaned_log_model)$p.value)

#plot(cleaned_log_model)
```
...
calc_loocv_rmse(cleaned_log_model)
```

- R^2, RMSE (multiple linear regression)
```{r}
summary(cleaned_log_model)$r.squared
summary(cleaned_log_model)$adj.r.squared
summary(cleaned_log_model)$coefficients
```