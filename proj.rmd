...
Use backwards and fowards AIC/BIC using the simple additive model.
```{r}
# back aic
...
```{r}
# use anova to compare the models created from AIC and BIC with the simple additive model
(back_pvalue = anova(back_model, full_add_model)[2, "Pr(>F)"])
(forward_aic_pvalue = anova(forward_aic, full_add_model)[2, "Pr(>F)"])
(forward_bic_pvalue = anova(forward_bic, full_add_model)[2, "Pr(>F)"])
```
- The model created from backwards AIC/BIC predicts better than the simple additive model with a p-value of `r back_pvalue`.
- The model created from forward AIC predicts better than the simple additive model with a p-value of `r forward_aic_pvalue`.
- The the model created from forward BIC predicts better than the simple additive model with a p-value of `r forward_bic_pvalue`.

```{r}
# use anova to compare the models created from AIC and BIC with each other
back_model$coef
forward_aic$coef
forward_bic$coef

anova(back_model, forward_aic)[2, "Pr(>F)"]
anova(back_model, forward_bic)[2, "Pr(>F)"]
anova(forward_aic, forward_bic)[2, "Pr(>F)"]
```
...
# calculate adjusted R^2
summary(int_model)$adj.r.squared
```
Model with all the available predictors as well as all possible 2-way interactions predicts better than the full additive model. 
